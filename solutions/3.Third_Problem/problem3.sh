...
olds=($(sort ../../palavras_antigas.lst | sed -r 's/([[:alnum:]])-([[:alnum:]])/\1traco\2/g' | sed "s/'/aspas/g" | tr -d '[:punct:]' | tr '[:upper:]' '[:lower:]' | uniq))
...
words=$(sed 's/ /\n/g' ../../O_cortico.txt | sed -r 's/([[:alnum:]])-([[:alnum:]])/\1traco\2/g' | sed "s/'/aspas/g" | tr -d '[:punct:]' | tr '[:upper:]' '[:lower:]' | sort | uniq -c | awk '{$1=$1};1')
