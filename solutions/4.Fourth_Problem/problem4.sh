...
words=($(sed -r 's/([[:alnum:]])-([[:alnum:]])/\1traco\2/g' ../../O_cortico.txt | sed 's/[^[:alnum:][:space:]]//g' | tr '[:punct:]' ' ' | cat))
...
sed 'y/áÁàÀãÃâÂéÉêÊíÍóÓõÕôÔúÚçÇ/aaaaaaaaeeeeiioooooouucc/' validas.txt | sed 's/ /\n/g' | tr '[:upper:]' '[:lower:]' | sort | uniq -c > vocabulario.txt

#Retira caracteres invalidos, recoloca o "-" de algumas palavras e troca a ordem para palavra e numero de repeticoes
awk '{$1=$1};1' vocabulario.txt | awk '{temp=$1; $1=$2; $2=temp; print}' | sed 's/ /,/' | sed 's/traco/-/g' > validas.txt
